...
{
    private int gridSizeX = 10;     //도시가로크기
    private int gridSizeZ = 10;     //도시 세로크기
    private float buildingSpacing = 15; //건물사이간격
    private float roadWidth = 5;         //도로폭
    ...
    private void OnGUI()
    {
        GUILayout.Label("Simple City Generator", EditorStyles.boldLabel);   //제목표시
        ...
        roadWidth = EditorGUILayout.FloatField("Road Width", roadWidth);    //도로폭 입력
        makeStatic = EditorGUILayout.Toggle("Make Static", makeStatic);     //Static설정
        GUILayout.Space(10);
        if (GUILayout.Button("Generate City"))                              //도시생성 버튼
        {
            GenerateCity();
        }
        if (GUILayout.Button("Clear City"))                                   //도시삭제버튼
        {
            ClearCity();
        }
    }
    private void CreateBuiIding(Vector3 position , Transform parent)
    {
    ...
    private void CreateRoad(Vector3 position , Transform parent)
    {
        ...
        renderer.material.color = new Color(0.3f, 0.3f, 0.3f);                      //회색
    ...
    private void ClearCity()        //도시삭제함수
    {
        GameObject city = GameObject.Find("City");  //city오브젝트찾기

        if(city != null)
        {
            DestroyImmediate(city);         //에디터에서 즉시삭제
        }
    }
    ...
    private void GenerateCity()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int z = 0; z < gridSizeZ; z++)
            {
                ...
                {
                    CreateBuiIding(position, buildingsParent.transform);
                }
            }
        }
    }    
